...
| Confirm the Zillow and map links | All buyer-contact actions currently route to the property’s Zillow page, and visit actions route to Google Maps. |
...
The current implementation does not include a database, login, or external content-management service. This keeps the one-property workflow inexpensive and reduces the number of systems that can fail. A browser-based editor can be added later if listing changes become frequent enough to justify it.
